...
using FMIBase.FMICore:
    fmi3ModelDescriptionModelExchange,
    fmi3ModelDescriptionCoSimulation,
    fmi3ModelDescriptionScheduledExecution,
    fmi3ModelDescriptionDefaultExperiment
using FMIBase.FMICore:
    fmi3ModelDescriptionModelStructure, fmi3ModelDescriptionDefaultExperiment
...
function fmi3LoadModelDescription(pathToModellDescription::String)
    ...
    @assert (
        ...
        "fmi3ReadModelDescription(...): Unknown entry for `variableNamingConvention=$(variableNamingConventionStr)`.",
    ]
    md.variableNamingConvention = (
        variableNamingConventionStr == "flat" ? fmi3VariableNamingConventionFlat :
    ...
    for node in eachelement(root)
        if node.name == "CoSimulation"
            ...
            md.coSimulation.providesDirectionalDerivatives =
                parseNode(node, "providesDirectionalDerivatives", Bool; onfail = false)
            md.coSimulation.providesAdjointDerivatives =
                parseNode(node, "providesAdjointDerivatives", Bool; onfail = false)
            md.coSimulation.hasEventMode =
                parseNode(node, "hasEventMode", Bool; onfail = false)
        ...
        elseif node.name == "ModelExchange"
            ...
            md.modelExchange.canSerializeFMUState =
                parseNode(node, "canSerializeFMUState", Bool; onfail = false)
            md.modelExchange.providesDirectionalDerivatives =
                parseNode(node, "providesDirectionalDerivatives", Bool; onfail = false)
            md.modelExchange.providesAdjointDerivatives =
                parseNode(node, "providesAdjointDerivatives", Bool; onfail = false)
        ...
        elseif node.name == "ScheduledExecution"
            ...
            md.scheduledExecution.needsExecutionTool =
                parseNode(node, "needsExecutionTool", Bool; onfail = false)
            md.scheduledExecution.canBeInstantiatedOnlyOncePerProcess =
                parseNode(node, "canBeInstantiatedOnlyOncePerProcess", Bool; onfail = false)
            md.scheduledExecution.canGetAndSetFMUState =
                parseNode(node, "canGetAndSetFMUState", Bool; onfail = false)
            md.scheduledExecution.canSerializeFMUState =
                parseNode(node, "canSerializeFMUState", Bool; onfail = false)
            md.scheduledExecution.providesDirectionalDerivatives =
                parseNode(node, "providesDirectionalDerivatives", Bool; onfail = false)
            md.scheduledExecution.providesAdjointDerivatives =
                parseNode(node, "providesAdjointDerivatives", Bool; onfail = false)
            md.scheduledExecution.providesPerElementDependencies =
                parseNode(node, "providesPerElementDependencies", Bool; onfail = false)
        ...
        elseif node.name == "DefaultExperiment"
            ...
            md.defaultExperiment.tolerance = parseNode(
                node,
                "tolerance",
                fmi3Float64;
                onfail = md.defaultExperiment.tolerance,
            )
...
function parseModelVariables(md::fmi3ModelDescription, nodes::EzXML.Node)
    ...
    for node in eachelement(nodes)
        ...
        modelVariables[index].canHandleMultipleSetPerTimeInstant =
            parseNode(node, "canHandleMultipleSetPerTimeInstant", Bool)
        ...
        if haskey(node, "initial")
            if typename ∉ ("Clock", "String", "Enumeration")
                modelVariables[index].initial =
                    stringToInitial(md, parseNode(node, "initial", String))
            else
    ...
    for node in eachelement(nodes)
        if haskey(node, "valueReference")
            ...
            elseif node.name == "ContinuousStateDerivative"
                ...
                stateValueRef =
                    modelVariablesForValueReference(md, derVar.derivative)[1].valueReference
