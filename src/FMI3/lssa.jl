        ...
        for (root, _, files) in walkdir(lsRootPath)
                for file in files
                        if file == "fmi-ls-manifest.xml"
                                ...
                                try
                                        lsObj = parseLayeredStandardManifest!(fmu.modelDescription, manifestPath)
...
function parseLayeredStandardManifest!(md::fmi3ModelDescription, pathToManifest::String)
        ...
        lsDescription = haskey(root, "fmi-ls-description") ? root["fmi-ls-description"] : "Unknown"
...
function parseLayeredStandardModelVariables!(md::fmi3ModelDescription, lsObj::FMIModelDescriptionLSSA, nodes::EzXML.Node)
...
function parseLayeredStandardModelStructure!(md::fmi3ModelDescription, lsObj::FMIModelDescriptionLSSA, nodes::EzXML.Node)
        for node in eachelement(nodes)
                list = nothing
